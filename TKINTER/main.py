...
def registrarI():
    ...
    if not validarGmail(email):
        messagebox.showerror("Error:", "Ingreso de correo erroneo")
        return
    if existeUsuario(email):  # 👈 usa la función que te paso abajo
...
def pantallaUsuario():
    ...
    ttk.Button(
        ...
        style="BotonMediano.TButton",
        width=25,
        command=lambda: cambiarPantalla(pantallaLogin),
    ).pack(pady=20)
...
def app():
    global raiz, framePrincipal, fuenteB
    ...
    style = ttk.Style()
    style.configure("BotonGrande.TButton", font=("Arial", 28, "bold"), padding=10)
